...
class WorkShop extends PolymerElement {
    ...
    return html`
      ...
      *{
      ...
      <template is="dom-if" if="{{requ}}"><cute-progress></cute-progress></template>
        ...
        <a href="cms/newshop" class="new"><paper-icon-button icon="add"></paper-icon-button></a>
        <a on-click="refresh" class="new"><paper-icon-button icon="cached"></paper-icon-button></a>
            <h3 id="h3"> کارگاه های اموزشی</h3>
            <div class="dl"></div>
            <template is="dom-repeat" items="[[posts]]">
            <div class="item">
                <h5>[[item.topic]]</h5>
                    <div class="options">
                    <a href="shop/[[item.id]]"><paper-icon-button icon="link" class="small"></paper-icon-button></a>
                    <a href="cms/shop/[[item.id]]"><paper-icon-button icon="assignment" class="small"></paper-icon-button></a>
...
<paper-toast id="log"></paper-toast>
  ...
  delete(e){
    ...
    this.$.request.url="http://api.anfas1.org/cms/workshop/delete/?id="+id+"&"+this.PEREMESSION()
...
window.customElements.define('workshop-cms', WorkShop);
